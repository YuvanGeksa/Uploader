...
// =========================
// User bisa upload ke repo milik mereka sendiri (atau repo lain yang mereka punya akses).
// Tidak ada whitelist. Karena ini situs publik, pertimbangkan menambahkan Access Code / rate limit di versi berikutnya.
...
async function uploadFilesContentsApi({ token, owner, repo, branch, files, commitMsg, log, onProgress }){
  // Upload via Contents API (PUT per file)
  // Note: This is simplest. For huge file counts, may be slow.
  let done = 0;
  ...
  async function worker(){
    while (idx < total){
      ...
      // Try read existing to include sha (update vs create)
      let sha = null;
      try{
        const existing = await ghFetch(token, `/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}?ref=${encodeURIComponent(branch)}`);
...
export default function Page(){
  ...
  const fullRepo = useMemo(() => `${(owner||"").trim()}/${(repo||"").trim()}`.replace(/^\/+|\/+$/g,""), [owner, repo]);
  const steps = useMemo(() => ([
    {n:1, t:"GitHub Access"},
    {n:2, t:"Select ZIP"},
    {n:3, t:"Review"},
  ...
  return (
    <div className="container">
      <div className="topbar">
        ...
        <div style={{opacity:.9}}>{fullRepo}</div>
      </div>

      <div className="surface">
        <div className="hero">
          <h1 className="h1">Upload to GitHub</h1>
          <p className="sub">One UI-inspired ZIP uploader. User cukup isi token + owner + repo + ZIP.</p>
        </div>

        <div className="stepper">
          {steps.map(s => (
            <div key={s.n} className={"pill" + (step===s.n ? " active":"")}>{s.n}. {s.t}</div>
          ))}
        </div>

        {err ? <div className="card"><div className="notice">{err}</div></div> : null}
        {ok  ? <div className="card"><div className="success">{ok}</div></div> : null}
  ...
  <div className="field">
    ...
    <input className="input" value={owner} onChange={(e)=>setOwner(e.target.value)} placeholder="Yopandelreyz" spellCheck={false} />
    <div className="hint">Contoh: <b>Yopandelreyz</b></div>
  </div>
  <div className="field">
    <div className="label">Repository</div>
    <input className="input" value={repo} onChange={(e)=>setRepo(e.target.value)} placeholder="test-upload" spellCheck={false} />
    <div className="hint">Contoh: <b>test-upload</b></div>
  </div>
</div>

<div className="hint">
  Hanya repository yang ada di <b>whitelist</b> yang bisa dipakai: {repo.trim()S.length ? repo.trim()S.join(", ") : "(whitelist kosong)"}
  <br/>
  Status: {isRepoAllowed ? <b style={{color:"var(--success)"}}>Allowed</b> : <b style={{color:"var(--danger)"}}>Not allowed</b>}
</div>

<div className="hint" style={{marginTop:10}}>
  Pastikan token kamu punya akses <b>write</b> ke repo yang dipilih.
</div>
        ...
        {step === 3 && (
          <div className="card">
            <div className="kv">
              <div className="k">Repo</div><div className="v">{owner.trim()}/{repo.trim()}</div>
            ...
            <div className="toggleRow">
              <label className="toggle">
                <input
                  ...
                  onChange={()=>setBranchMode("default")}
                />
        ...
        <div className="logWrap">
          ...
          <div className="log" ref={logRef}>
            {logLines.length ? logLines.map((x,i)=>(
              <div key={i} className={x.kind || ""}>{x.line}</div>
            )) : <div style={{opacity:.7}}>[{nowTime()}] Ready.</div>}
          </div>
        </div>
          ...
          <div className="ctaBar">
            {step > 1 ? (
              ...
              <button className="btnGhost" onClick={()=>{
                setToken(""); setFiles([]); setZipName(""); setZipSize(0); setLogLines([]);
                setOwner(DEFAULT_OWNER);
                setRepo(DEFAULT_REPO);
                setErr(""); setOk(""); setStep(1);
